...
export function useZaps(
  ...
  const zap = async (amount: number, comment: string) => {
    ...
    try {
      ...
      // Create zap request with proper parameters based on event kind
      // For addressable events (30000-39999), use the event object to get 'a' tag
      // For all other events, use the event ID to get 'e' tag
      const eventParam: Event | string = (actualTarget.kind >= 30000 && actualTarget.kind < 40000)
        ? actualTarget
        : actualTarget.id;
